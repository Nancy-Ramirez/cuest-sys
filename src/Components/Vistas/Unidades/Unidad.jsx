...
const GestionarUnidades = () => {
  ...
  useEffect(() => {
    async function getInfo() {
      const url = "http://localhost:8000/api/unidad/listar";
  ...
  const FuncionEliminar = () => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        Swal.fire("Eliminado", "La unidad se ha removido", "success");
      }
    });
  };
                    ...
                    .map((unidad) => {
                      return (
                        <tr className="bg-green-200 border-b dark:bg-gray-800 dark:border-gray-700  hover:bg-coll6 hover:text-white dark:hover:bg-gray-600">
                          ...
                          <td className="px-6 py-4">{unidad.grado}</td>
                          <td className="px-6 py-4">{unidad.materia}</td>
